(() => {
  ...
  const SITEMAP = "https://api.buk1t.com/json/site-map.json";
  ...
  async function loadTargets() {
    ...
    try {
      ...
      const fromMap = subs.filter(s => allowed.has(s)).map(s => ({ name: s, url: `https://${s}.${DOMAIN}/` }));
  ...
  async function fetchRepoInfo(repoUrlOrSlug) {
    ...
    if (!res.ok) return { slug, pushed_at: null, html_url: `https://github.com/${slug}` };
    const data = await res.json();
    return { slug, pushed_at: data.pushed_at || null, html_url: data.html_url || `https://github.com/${slug}` };
  }
  ...
  async function main() {
    ...
    try {
      const needed = uniqBy(
        cards
          .map((c) => {
            const key = c.t.repoKey || c.t.name;
            return { key, repo: REPO_MAP[key] };
          })
          .filter((x) => x.repo),
        (x) => x.repo
      );
      ...
      for (const c of cards) {
        ...
        c.st.appendChild(repoPill(info?.html_url || `https://github.com/${slug}`, info?.pushed_at || null));
      }
    } catch {}
    ...
    metaEl.textContent = `checked: ${checkedAt.toLocaleString()}${newestPush ? ` • latest repo push: ${relTime(new Date(newestPush).toISOString())}` : ""}`;
  }
